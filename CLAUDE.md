...
**Known upstream regression affecting subagent MCP access.** As of 2026-05-12, Claude Code v2.1.46+ has a regression ([anthropics/claude-code#25200](https://github.com/anthropics/claude-code/issues/25200)) that prevents custom subagents from reaching MCP servers. Every dm/-touching subagent (world-state, librarian, revelation, bookkeeper, mythic for thread CRUD) is affected — calls to `mcp__dm-fs__*` tools fail at dispatch time. The asymmetry boundary itself is intact (the narrator still cannot read `dm/` directly), but the crossing mechanism is broken until upstream fix lands. See `docs/known-limitations.md` for the full impact map, paths forward, and recovery procedure. Until then, route attempts to dm/-touching subagents will likely return degraded responses; surface that fact in narration rather than fabricating around it.
